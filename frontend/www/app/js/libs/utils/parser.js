...
var Parser = (function (scope) {
    ...
    function addUnits(a, b) {
        if (a == 1) return b;
        if (b == 1) return a;
        if (a != b) throw new InvalidUnitException();
        else return a;
    }
    ...
    function Parser(validateUnits, nodes) {

        this.validateUnits = validateUnits ? validateUnits : false;
        this.success = false;
        this.errormsg = "";
        this.expression = "";
        ...
        // One more dirty hack for validating units
        if (this.validateUnits) {
            this.ops2["+"] = addUnits;
            this.ops2["-"] = subUnits;
            for (property in nodes) this.consts[property] = nodes[property];
        }
        ...
        parse:function (expr) {
            ...
            while (this.pos < this.expression.length) {
                if (this.isOperator()) {
                    ...
                    // Dirty hack for validating units :)
                    if (this.validateUnits) {
                        this.tokennumber = 1;
                    }
...
var FormulaWorkaround = new Object();

FormulaWorkaround.topologicalSort = function (collection) {
    var graph = new Object();
    graph.empty = true;
    for (offset in collection.models) {
        graph.empty = false
        var model = collection.models[offset];
        graph[model.id] = { dependencyIds:model.attributes.dependencyIds ? model.attributes.dependencyIds : [] };
    }
    ...
    function iterate(arr, callback) {
        for (property in arr) {
            callback(arr[property], property);
        }
    }
    ...
    function processStartingPoint(nodeId) {
        ...
        iterate(graph[nodeId].dependencyIds, function (e) {
            graph[e].indegrees--;
        });
